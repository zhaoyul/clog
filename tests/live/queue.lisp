...
(test live-queue/overflow/disconnect-is-terminal
  (let ((queue
          (clog-live:make-bounded-queue
...
(test live-queue/close/is-idempotent-clears-buffer-and-wakes-all-waiters
  ...
  (let ((queue (clog-live:make-bounded-queue :capacity 2)))
    ...
    (is (= 0 (clog-live:queue-size queue)))
    (is (eq :closed
            (nth-value 1 (clog-live:dequeue queue :timeout 0))))))
                      ...
                      (let ((done nil))
                        (bordeaux-threads:with-lock-held (result-lock)
                          (when (gethash item seen)
                            (unless failure
                              (setf failure :duplicate-item)))
                          (setf (gethash item seen) t)
                          (decf remaining)
                          (when (minusp remaining)
                            (unless failure
                              (setf failure :too-many-items)))
...
(defun lv-040-overflow-producer (queue producer capacity failure-lock failure-cell)
  "Fill QUEUE aggressively and record any observed capacity violation."
         ...
         (threads
           (loop for producer below 8
                 ...
                 (let ((producer producer))
                   (bordeaux-threads:make-thread
                    (lambda ()
                      (lv-040-overflow-producer
                       queue producer capacity failure-lock failure-cell))
                    :name "lv040-overflow-producer")))))
    (dolist (thread threads)
      (bordeaux-threads:join-thread thread))
    (is (null (car failure-cell))
        "Observed queue size above capacity: ~S"
        (car failure-cell))
